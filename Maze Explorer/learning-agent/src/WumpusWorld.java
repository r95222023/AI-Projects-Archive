...
// updateFrontiers update the frontier level at each move. The frontier level for the initial world state is plot below.
// 0 represents a visited cell, 1s represent the first layer of frontiers, 2s represent  second layer frontiers and so on.
//
//                -----------------------
//               |     |     |     |     |
//               |   3 |   4 |   5 |   6 |
...
//               |   0 |   1 |   2 |   3 |
//                -----------------------
    public void updateFrontiers(int level,int X, int Y) {
        ...
        for (int[] neighbor : neighbors) {
            ...
            int ny = neighbor[1];
            if(level==0){
                cells[nx][ny].frontier = 1;
                updateFrontiers(1, nx, ny);
            } else if (cells[nx][ny].frontier - level > 1) {
                cells[nx][ny].frontier = level+1;
                updateFrontiers(level+1, nx, ny);
            }
        }
    }
...
//                        cells[i][j].wumpusExists = 0;
    ...
    public void tryFrontierConfiguration(int X, int Y, int configuration) {
        ...
        switch (configuration) {
            ...
            case 4:
                // found gold
                if (pits.contains(posStr) || (wumpus.contains(posStr) && !wumpusKilled) || cells[X][Y].safe == -1) {
                    valid = -1;
                }
    ...
    public boolean verifyWorld(ArrayList<int[]> frontiers) {
        if (wumpusFound > 1 || pitFound > 2) return false;
        for (int[] frontier : frontiers) {
            ...
            for (ArrayList<int[]> jumps : getOneJumps(fx, fy)) {
                ...
                if (cells[nx][ny].visited == 1) {
                    for (int[] jump : jumps) {
                        int jx = jump[2];
                        int jy = jump[3];
                        if (cells[jx][jy].pit == 4 || pits.contains(jx + "-" + jy)) {
                            hasPit = true;
                        }

                        if (cells[jx][jy].wumpus == 4 || wumpus.contains(jx + "-" + jy)) {
                            hasWumpus = true;
                        }
                    ...
                    if ((breeze && !hasPit) || (stench && !hasWumpus && !wumpusKilled)) {
                        return false;
                    }
                }
