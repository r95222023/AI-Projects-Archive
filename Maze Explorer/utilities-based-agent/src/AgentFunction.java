...
class AgentFunction {
    ...
    int nextDestination(WumpusWorld wumpusWorld) {
        WumpusCell[][] cells = wumpusWorld.cells;

        ForwardSearch search = new ForwardSearch();
        SearchResult result = search.fwdSearch(agentXPos, agentYPos, direction, wumpusWorld);
...
//            destX = result.destX;
//            destY = result.destY;
        } else if (result.utilities.size() * result.maxUtility / result.multiplicity > 950) { // Check if it's worthy to explore an unknown cell.
//            destX = result.destX;
//            destY = result.destY;
            ...
            if (target[0]!=-1){
                ...
                SearchResult wumpuslessResult = search.fwdSearch(agentXPos, agentYPos, direction, wumpusWorld);
                // Compare the utilities between worlds with and without a wumpus.
                if (wumpuslessResult.maxUtility > 0 && (wumpuslessResult.maxUtility / wumpuslessResult.multiplicity - result.maxUtility / result.multiplicity) > 1200) {
                    destX = -1; // Shoot arrow!
                    destY = -1;
                    huntWumpus = false;
                } else {
        ...
        int acttion = nextAction(destX, destY, wumpusWorld);
        return acttion;
    }
